...
int main(int argc, char *argv[]) {
	...
	printf("package GESTE_Fonts.%s is\n\n", fontName);
	printf("   Font : constant Bitmap_Font_Ref;\n");
	printf("private\n\n");

	printf("   %sBitmaps : aliased constant Font_Bitmap := (\n  ",
               fontName);

        // Find the maximum width and height for this font
	for(i=first, j=0; i<=last; i++, j++) {
        ...
        int glyph_width  = max_width;
        int glyph_height = face->size->metrics.height >> 6;
        int bytes_per_glyph =
            ((glyph_width * glyph_height) + (glyph_width * glyph_height) % 8) / 8;
		...
		table[j].yOffset      = 1 - g->top;
                ...
                for(y=0; y < glyph_height; y++) {
                    for(x=0; x < glyph_width; x++) {
                        bit_cnt++;
                        px = x - g->left; // xOffset
                        py = y - (1 - g->top + max_height);

                        if (px < 0 || py < 0 ||
                            px >= bitmap->width || py >= bitmap->rows)
                        {
                            enbit(0);
                        } else {
                            byte = px / 8;
	...
	printf("   Font_D : aliased constant Bitmap_Font :=\n");
        printf("     (\n");
        printf("      Bytes_Per_Glyph => %d,\n", bytes_per_glyph);
        printf("      Glyph_Width     => %d,\n", glyph_width);
        printf("      Glyph_Height    => %d,\n", glyph_height);
        printf("      Data            => %sBitmaps'Access);\n", fontName);
        printf("   Font : constant Bitmap_Font_Ref := Font_D'Access;\n");
	printf("end GESTE_Fonts.%s;\n", fontName);
